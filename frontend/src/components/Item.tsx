        ...
        <Card className='relative cursor-pointer' imgSrc="https://miro.medium.com/v2/resize:fit:800/1*SIpSf-asJAUkS4cOumEqeg.png">
            ...
            <div className="flex">
                ...
                <label className="relative inline-flex items-center mb-5 cursor-pointer" >
                    <input title="toggle"  disabled= {item.state.id == 4 && true}  name="toggle" type="checkbox" value="" checked={check} onChange={handleChange} className="sr-only peer" />
                    <div className="w-11 h-6 bg-red-600 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 dark:peer-focus:ring-blue-800 rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-blue-600 peer-disabled:bg-gray-400">
                    </div>         
                </label>
                {item.state.id == 4 && 
                    <span className="ml-3 text-center align-items text-sm ">Cannot change the state when the vm shutdown</span>}
            </div>
        </Card >
    );
}
